...
const People = () => {
  ...
  const getPeople = async (reset = false, selectedPage = page) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `/person/popular?page=${selectedPage}&limit=15`
      );
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      console.log(`People - Distance from bottom: ${distanceFromBottom}px`);
      ...
      if (distanceFromBottom < 300) {
        ...
        timeout = setTimeout(() => {
          console.log("Loading more people...", { page: page + 1 });
    ...
    setTimeout(() => {
      ...
      if (documentHeight <= windowHeight * 1.2 && !loading && hasMore) {
        console.log("People - Content does not fill page, loading more...");
        setPage((prevPage) => prevPage + 1);
      }
    }, 500);
