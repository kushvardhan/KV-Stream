...
const Movie = () => {
    ...
    const getMovies = async (reset = false, selectedCategory = category, selectedPage = page) => {
        if (loading || !hasMore) return;
        setLoading(true);

        try {
            const { data } = await axios.get(`/movie/${selectedCategory}?page=${selectedPage}`);
    ...
    <div className="flex flex-wrap justify-center items-stretch gap-6 px-4 sm:px-6 w-full">
        {movies.length === 0 && loading ? (
            ...
            movies.map(item => (
                <Cards key={item.id} data={item} category="movie" hideDetails={false} />
            ))
        )}
    </div>
