    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 bg-gradient-to-t select-none from-black via-black/40 to-transparent p-3 flex flex-col justify-end opacity-100 group-hover:opacity-100 transition-opacity duration-300">
                <h2 className="text-sm font-bold text-white truncate">{data.title || data.name}</h2>
                <p className="text-xs text-gray-300 leading-tight mt-1">{shortOverview}</p>
                
                {!hideDetails && (
                    <div className="flex items-center select-none space-x-1 mt-1">
                        <i className={`${mediaType === "movie" ? "ri-movie-2-fill text-red-400" : "ri-tv-fill text-blue-400"} text-sm`}></i>
                        <p className="text-xs text-gray-400 uppercase font-bold tracking-wide">{mediaType}</p>
                    </div>
                )}
            </div>
        </Link>
